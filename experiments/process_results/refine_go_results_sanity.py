...
go_all = pd.read_csv('../results/old_GO_score.csv', index_col = 0)
go_wgcna = pd.read_csv('../results/new_wgcna_go_score.csv', index_col = 0)
...
for data_set in data_sets:
        for prototype, center_dim, data_dim in center_methods:
                for fold in range(5):
                        wgcna_query = (go_wgcna["Fold"] == str(fold)) &\
                                      (go_wgcna["Data Set"] == data_set)
                        if np.sum(wgcna_query) > 0:
                                wgcna_total = go_wgcna[wgcna_query]["GO Significance"].sum()
...
go_better.to_csv('../results/new_refined_GO_score.csv')
...
mean_go_data.to_csv('../results/new_refined_mean_go_score.csv')
simple_mean_go_data.to_csv('../results/new_refined_simple_mean_go_score.csv')

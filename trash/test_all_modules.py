...
def loso_test(ds, fold_number):
    ...
    if fold_number != 'all':
        if ('tolerant' in list(ds.metadata['label'])) or ('susceptible' in list(ds.metadata['label'])): #no log normalization for salmonella
            processes=(partition_5fold, hf, std, svm_model, bsr)
        else:    
            processes=(partition_5fold, hf, log2, std, svm_model, bsr)
    else:
        if ('tolerant' in list(ds.metadata['label'])) or ('susceptible' in list(ds.metadata['label'])): #no log normalization for salmonella
            processes=(hf, std, svm_model, bsr)
        else:
            processes=(hf, log2, std, svm_model, bsr)
    ...
    fold_test_bsr = bsr.collapse_results()['class_pred_scores']
...
if __name__ == '__main__':
    ...
    for module_type in os.listdir(dir_path): #center type
        ...
        folder_path = f'{module_type_dir}/all'

        for dataset_name in os.listdir(folder_path):
            if 'salmonella' in dataset_name:
...
svm_results.to_csv(f'svm_score_all.csv')
